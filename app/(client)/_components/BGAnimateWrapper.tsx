...
interface Props {
  ...
  sectionName?: "hero" | "cta";
}

function BGAnimateWrapper({ children, sectionName = "hero" }: Props) {
  const mouseX = useMotionValue(0);
  const mouseY = useMotionValue(0);

  const bgTranslateX = useTransform(mouseX, [-500, 500], [-10, 10]);
  const bgTranslateY = useTransform(mouseY, [-500, 500], [-10, 10]);
  const fgTranslateX = useTransform(mouseX, [-500, 500], [10, -10]);
  const fgTranslateY = useTransform(mouseY, [-500, 500], [10, -10]);
      ...
      style={{
        ...
        background:
          sectionName === "hero"
            ...
            : "",
        height: sectionName === "hero" ? "max(620px, calc(92vh - 49px))" : "",
        position: "relative",
      }}
      className={
        sectionName === "hero"
          ? "hero-clip-mobile sm:hero-clip-tablet xl:hero-clip-desktop"
          : "bg-gradient-to-l from-cyan-500 to-blue-500 cta-clip-mobile sm:cta-clip-tablet xl:cta-clip-desktop h-[60vh] lg:h-[530px] text-white relative"
      }
    >
      {sectionName === "hero" ? (
        <svg
...
export default BGAnimateWrapper;
